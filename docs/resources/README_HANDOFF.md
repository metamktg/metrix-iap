...
sidebar was a 56 px rail and a workspace map (change log entry 6 — withdrawn the same day on owner review: entry 7 is the collapsible sidebar whose pages disclose on a 180 ms dwell or focus, purposes as tooltips, inline notes back behind reveals; PR #189, `c85b55c`; and entry 8, the same day again, is the third pass — nothing in the sidebar expands: a click on a section navigates at once and a 0.7 s rest slides its pages out to the right as a flow-chart branch, PR #191, `05eb66b`, live as deployment `329ef7e0` with all 130 assets matching a local build of main); the KPI hover chart is drawn
