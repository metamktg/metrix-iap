...
## 9. The brief surface — 8 of 34 fields reached the screen (2026-09-01)

Second half of the owner report: the real value Metrix produces reads as
hidden in briefs too. Measured before touching anything.

### What the audit found

The generation engine persists **34 fields** into every brief's `payload`.
The Brief Builder page named **8**. Verified three ways: the `full_brief`
accessors are key-by-key, nothing renders a section by iterating its entries,
and each missing field was grepped across the whole `pages/metrix/creative`
tree.

Unlike §8, this was not text behind a click. It was text **generated by an LLM
call, written to the database, and rendered nowhere**:

| Section | Never reached the screen |
|---|---|
| copy_architecture | `problem_agitation_or_value_setup`, `product_solution`, `proof` — **three of the five copy blocks** |
| testing_framework | `isolated_variable`, `control_reference`, `learning_objective`, `matrix_position` |
| strategic_foundation | `target_icp`, `avatar_basis`, `concept_code`, `design_system`, `cta_type`, `performance_benchmark` |
| creative_specifications | `production_requirements`, `placement`, `creative_name` |
| brief_metadata | `generated_at`, `model` |

A brief promises a copywriter the problem setup, the product solution and the
proof. They were given a hook and a CTA.

### Built

- **Copy architecture** panel — setup / solution / proof, in narrative order.
- **Testing framework** — what the brief isolates, against which control, to
  learn what. Without those three a "test" is just another ad.
- **Target + benchmark** on the opening panel, carrying the generator's own
  honesty label: an `avatar_basis` of `exploratory` renders a warning chip
  (*"exploratory — no historical avatar data"*), because the generator
  deliberately refuses to attach a real ICP to a padded matrix column and the
  UI must not imply one.
- **Concept, matrix cell, placement, CTA type, design system, creative name**
  in the spec row; **generated date + model** as provenance under the brief id.
- **The emailed brief now matches the on-screen brief** — it previously
  carried the direction and variable descriptors only, so a writer receiving
  one got neither the copy architecture nor the test design.

### A duplication caught mid-build, and corrected

The first draft rendered `strategic_foundation.data_insight` as a new
"Evidence" block. It is already on screen: `metrixSeedAssembly` maps
`data_insight` onto `DraftBrief.human_direction`, which the "Why this brief
exists" panel renders. The block would have printed the same sentence twice,
in the UI and in the email. Removed from both, and the reason is recorded at
both sites so it is not re-added.

### Coverage

**34 of 34** fields now reach the screen (9 of them via the top-level
`DraftBrief` fields the adapter lifts out of `brief_metadata` /
`strategic_foundation`, the rest read directly from `full_brief`).

### One rendering defect found in the browser

Inline label spans used a right margin for separation, which gives a visual
gap but leaves no separator in the text stream — the page read
`BENCHMARKBeat C2B CPA of $9.13` to copy-paste and to a screen reader. Five
sites now carry an explicit space. Browser-confirmed: `TARGET
ICP_BOOK0_C2_TimePoorLearner`, `BENCHMARK Beat C2B CPA of $9.13…`.
