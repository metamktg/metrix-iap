...
3. **Upload both CSVs** from the checklist. Expected: each file validates on arrival,
   ...
   explicit Review before it closes. Nothing runs yet.
4. **Run analysis** from the Analysis command center: choose the window (it defaults
   to the latest date in the data) and press Run. Expected: a running row with
   progress, then success. Warnings, if the export lacked columns, appear in the run
