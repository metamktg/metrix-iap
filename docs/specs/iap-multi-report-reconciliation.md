...
| Control period must be the run window for whole-period sources and totals rows (daily sources are summed from window-filtered rows); a rejected candidate is recorded with its reason and the ledger names the export that fixes it | Amendment §3 compatibility key (reporting period); live finding 2026-09-03 (totals row of a 30-day pivot used for a 1–30 window → 28.54% where the same-window read was 59.3%) | `reconciliation.ts` `periodFit` inside `buildTruth` (`window` from the run), `TruthSet.rejected`, `[Truth] Rejected …` notes | implemented |
