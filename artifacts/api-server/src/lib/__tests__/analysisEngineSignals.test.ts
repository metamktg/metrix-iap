...
async function waitForRunToSettle(): Promise<Record<string, any>> {
  for (let i = 0; i < 200; i++) {
    const run = db.get("manual_analysis_runs")![0];
...
describe("startManualAnalysis · source guard", () => {
  it("refuses accounts owned by another pipeline (importer/live)", async () => {
    ...
    // Nothing was deleted or written for the foreign-owned account.
...
describe("startManualAnalysis · signal-table output", () => {
  it("writes the demographic/placement signal rows the UI and strategy evidence read", async () => {
    seedManualAccount();
    stageBothCsvs();
    // A creative uploaded BEFORE analysis, mapped to a real ad name — the
    // run must backfill its link onto the ads registry it creates.
    db.get("manual_imports")!.push({
      id: "imp_creative",
      account_id: ACCOUNT,
      kind: "creative_asset",
      filename: "ugc_testimonial_v1.mp4",
      content: stagedHex("fake"),
      ad_names: ["UGC_Testimonial_v1"],
    });
    ...
    // Placements surface: v3-scoped placement×platform signal, importer shape.
    ...
    // Loop status registers the analysis stage.
    const loop = db.get("iap_runs")!;
    expect(loop).toHaveLength(1);
    expect(loop[0]!["stage"]).toBe("analysis_core");
    expect(loop[0]!["status"]).toBe("complete");

    // The account is promoted honestly, and the pre-uploaded creative got
    // linked onto the ads registry created by the run.
    expect(db.get("ad_accounts")![0]!["status"]).toBe("configured");
    const ad = db.get("ads")!.find((r) => r["ad_name"] === "UGC_Testimonial_v1")!;
    expect(ad).toBeTruthy();
    expect(String(ad["creative_asset_url"])).toContain("imp_creative");
    expect(ad["asset_servable"]).toBe(true);
  });

  it("re-running replaces the signal rows instead of duplicating them", async () => {
    seedManualAccount();
    stageBothCsvs();
    await startManualAnalysis(ACCOUNT, "all", "tester");
    await waitForRunToSettle();
    const firstCount = db.get("demographic_signal")!.length;
    ...
    expect(db.get("demographic_signal")!.length).toBe(firstCount);
    expect(db.get("placement_signal")!.length).toBe(2);
  });
});
