# Command Deck Design System design system
...
workspace package (`@workspace/command-deck`): other artifacts depend
...
- Tokens extracted from the Command Deck instrument-panel mockup
  (`artifacts/mockup-sandbox/.../analysis-overview/CommandDeck.tsx`). **Dark is
  the source-native mode**; light is derived. The preview defaults to dark and
  has a Light/Dark toggle in the sidebar header. The mono face (`Share Tech
  Mono`) is a first-class brand element for labels, codes, and metric readouts.
- Composition guidelines derived from the mockup are shown on the Overview page.
...
`@workspace/command-deck/components/...`. Never use a `@/` alias inside
...
for `@workspace/command-deck` and renders mockups using this package's
theme and components.

## Consuming this package

Never copy token values, component source, hooks, or these docs into a consuming
artifact. Add `@workspace/command-deck` as a `workspace:*` dependency,
run `pnpm install`, and import directly from this package.

Read only the guides required by the current task:

- Building or styling web UI: `artifacts/command-deck/docs/consuming-web.md`
- Building or styling Expo UI: `artifacts/command-deck/docs/consuming-expo.md`
- Replacing an existing or scaffolded web theme/component library:
  `artifacts/command-deck/docs/migrating-web.md`
- Replacing existing or scaffolded Expo theme/hooks/components:
  `artifacts/command-deck/docs/migrating-expo.md`
