# Migrating Expo UI to Command Deck Design System

Read `artifacts/command-deck/docs/AGENTS.md` and
`artifacts/command-deck/docs/consuming-expo.md` first. Use this guide
...
- Rewrite color/theme imports to
  `@workspace/command-deck/lib/native-theme` and
  `@workspace/command-deck/hooks/use-colors`.
- Rewrite the root layout to use
  `@workspace/command-deck/hooks/use-fonts` while preserving its
  SplashScreen gating.
...
and hook directly from `@workspace/command-deck` and retains only
