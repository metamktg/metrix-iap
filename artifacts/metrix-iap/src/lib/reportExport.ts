...
<div class="page">
  <div class="masthead">
    ...
    <div class="meta">${esc(model.accountName)} · ${esc(model.platform)}${model.windowLabel ? ` · Window ${esc(model.windowLabel)}` : ""} · Generated ${esc(dateStr)}</div>
  </div>
  ${sectionsHtml}
...
async function downloadPdf(model: ReportModel, filename: string): Promise<void> {
  ...
  doc.setTextColor(107, 114, 128);
  const dateStr = model.generatedAt.toLocaleDateString("en-US", { year: "numeric", month: "long", day: "numeric" });
  const windowPart = model.windowLabel ? ` · Window ${model.windowLabel}` : "";
  doc.text(`${model.accountName} · ${model.platform}${windowPart} · Generated ${dateStr}`, margin, y);
  y += 10;
