    ...
    <Dialog open={open} onOpenChange={(v) => !v && onClose()}>
      <DialogContent className="max-w-3xl bg-[hsl(222_61%_6%)] border-border/50">
        <DialogHeader className="text-left space-y-1">
          ...
          <DialogTitle className="text-body-lg font-semibold text-foreground">{title} — avatar × placement</DialogTitle>
              ...
              <table className="w-full min-w-[500px] border-collapse">
                <thead className="sticky top-0 bg-[hsl(222_55%_7%)] z-10">
                  <tr className="border-b border-border/40">
                    <th className="text-left text-label-xs font-mono uppercase tracking-widest text-muted-foreground/60 font-semibold px-2.5 py-2">
                      Avatar segment
                    </th>
                    {placements.map(({ row: p }) => (
                      <th key={p.Placement + p.Platform} className="text-center text-label-xs font-mono uppercase tracking-wide text-muted-foreground/60 font-semibold px-2 py-2 min-w-[76px]">
                        <div className="normal-case">{p.Placement}</div>
                        <div className="text-label-xs text-muted-foreground/50 capitalize">{p.Platform}</div>
                      </th>
                    ))}
                    <th className="text-right text-label-xs font-mono uppercase tracking-widest text-primary/70 font-semibold px-2.5 py-2">
                      Blended
                    </th>
                  </tr>
                </thead>
                <tbody>
                    ...
                    return (
                      <tr key={seg.key} className="border-b border-border/20">
                        <td className="px-2.5 py-2">
                          <div className="text-label font-medium text-foreground">{seg.age}</div>
                          <div className="text-label-xs text-muted-foreground/60 capitalize">{seg.gender}</div>
                        </td>
                        {placements.map(({ row: p }) => (
                          <td
                        ...
                        <td className="px-2.5 py-2 text-right tabular-nums">
                          <div className="text-label font-semibold text-foreground">
                            {blended.display} <span className="text-label-xs font-normal text-muted-foreground/60">{metricLabel}</span>
                          </div>
                          <div className="text-label-xs text-muted-foreground/60">{usd(seg.totals.spend, 0)} · {num(seg.totals.results)} res</div>
                        </td>
                      </tr>
                    );
                  })}
                  ...
                  <tr className="border-t border-border/40 bg-white/[0.015]">
                    <td className="px-2.5 py-2">
                      ...
                      <div className="text-label-xs text-muted-foreground/60">placement marginals · account level</div>
                    </td>
                    {placements.map(({ row: p, totals }) => {
                      ...
                      return (
                        <td key={p.Placement + p.Platform} className="px-2 py-2 text-center tabular-nums">
                          ...
                          <div className="text-label-xs text-muted-foreground/60">{usd(p["Amount spent (USD)"], 0)}</div>
                        </td>
                      );
                    })}
                    <td className="px-2.5 py-2" />
