    ...
    <DropdownMenu open={open} onOpenChange={setOpen}>
      <DropdownMenuTrigger asChild>
        ...
        >
          ...
          <span>{label}</span>
          <ChevronsUpDown className="w-3.5 h-3.5 text-muted-foreground/75 shrink-0" />
        </button>
      </DropdownMenuTrigger>
          ...
          return (
            ...
            >
              <div className={cn("w-5 h-5 rounded border flex items-center justify-center shrink-0", isActive ? "bg-primary/15 border-primary/25" : "bg-foreground/[0.04] border-border/30")}>
                <Briefcase className={cn("w-3.5 h-3.5", isActive ? "text-interactive" : "text-muted-foreground/75")} />
              </div>
