...
// Link steps navigate correctly, the real CSV format panel renders with
// its data, and the final step opens the real AddAccountDialog (same
// component the normal "Add Account" button uses — not a stand-in).
...
describe("OnboardingWizard", () => {
  ...
  it("Next advances welcome → prepare, showing live CSV format panels", async () => {
    ...
    expect(screen.getByText(/Required columns — Demographics CSV/i)).toBeTruthy();
    expect(screen.getByText(/Required columns — Placements CSV/i)).toBeTruthy();
  });
  ...
  it("Next from prepare reaches the link step, and Add Ad Account opens the real AddAccountDialog", async () => {
    ...
    // The real AddAccountDialog's choose step must appear.
  ...
  it("completing the manual-account flow from within the wizard calls selectAdAccount", async () => {
    const user = userEvent.setup();
    renderWizard();
    ...
    await user.click(screen.getByRole("button", { name: /Done — open account/i }));
