                ...
                return (
                  ...
                  >
                    <div className="flex items-center gap-1.5 mb-1.5">
                      {g.cellIds.map((c) => (
                        <span key={c} className="text-label-xs font-mono text-muted-foreground/70 border border-border/40 px-1 py-0.5 rounded leading-none">{c}</span>
                      ))}
                    </div>
                    <p className="text-body font-semibold text-foreground leading-tight">{g.name}</p>
                    ...
                    <div className="flex items-center justify-between mt-1.5 text-label text-muted-foreground/70 tabular-nums">
                    ...
                    <div className="mt-3 pt-3 border-t border-border/20 flex items-center gap-1.5 flex-wrap">
                      ...
                      {linked.length ? (
                        ...
                        <span className="text-label text-muted-foreground/70">No pillar linked yet</span>
                      )}
                    </div>
                  </button>
                );
                  ...
                  <div className="space-y-1.5">
                    {detail.cells.map((r) => (
                      <div key={r.cell_id + r["Result type"]} className="flex items-center justify-between gap-2">
                        <span className="text-label text-muted-foreground/70">{r.cell_id} · {eventLabel(r["Result type"])}</span>
                        <span className="text-label text-foreground/80 tabular-nums">{fmtNum(r.Results)} · {r.CPA_result != null ? fmtUSD(r.CPA_result) : "—"} CPA</span>
                      </div>
                    ))}
                  </div>
                </DrawerField>
                <DrawerField label="Variable codes">
                  <div className="space-y-2">
                    {detail.cells.filter((r, i, arr) => arr.findIndex((x) => x.cell_id === r.cell_id) === i).map((r) => (
                      <div key={r.cell_id}>
                        <span className="text-label-xs font-mono text-muted-foreground/70">{r.cell_id}</span>
                ...
                {(getStrategyData(seed, adAccountId)?.message_pillars ?? [])
                  ...
                  .map((p) => (
                    <DrawerField key={p.id} label={`Pillar · ${p.label}`}>
                      ...
                      <p className="mt-1 text-label text-muted-foreground/70">{p.why_it_matters}</p>
                    </DrawerField>
                  ))}
