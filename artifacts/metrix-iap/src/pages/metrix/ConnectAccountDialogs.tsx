    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-md">
        <DialogHeader>
          ...
          <DialogTitle className="text-body-lg font-semibold">Connect Meta Ad Account</DialogTitle>
          <DialogDescription className="text-body text-muted-foreground/80 leading-relaxed">
          ...
          {[
            ...
            <div key={title} className="flex items-start gap-3 p-3 rounded-lg border border-border/40 bg-white/[0.02]">
              ...
              <div className="min-w-0">
                ...
                <p className="text-label text-muted-foreground/85 leading-relaxed mt-0.5">{desc}</p>
              </div>
            </div>
          ))}
        </div>
    ...
    <div className="space-y-2">
      ...
      {staged ? (
        <div className="flex items-center gap-2 p-2 rounded-md border border-emerald-400/20 bg-emerald-400/[0.05]">
          ...
          <span className="text-label text-foreground/80 truncate">{staged.filename}</span>
          <span className="text-label-xs font-semibold uppercase tracking-wide text-emerald-400/90 ml-auto shrink-0 mr-1">Staged</span>
          <button
        ...
        <>
          ...
          >
            ...
            {file ? (
              ...
              <span className="text-label text-muted-foreground/80">Choose a .csv file (max 8 MB)</span>
            )}
          </button>
          {file && uploadPct === null && (
      ...
      {error && (
        <div className="flex items-start gap-2 p-2.5 rounded-lg border border-red-400/25 bg-red-400/[0.06]">
          ...
          <p className="text-label text-red-300 leading-relaxed">{error}</p>
        </div>
      )}
    </div>
  );
  ...
  return (
    <Popover open={open} onOpenChange={setOpen}>
      <PopoverTrigger asChild>
        <button
          className="flex items-center gap-1.5 h-7 px-2.5 rounded bg-white/[0.03] border border-border/50 text-label text-foreground hover:border-primary/40 transition-colors cursor-pointer"
...
function MatchMethodBadge({ method }: { method?: "id" | "fuzzy" | "guess" | null }) {
  ...
  return (
    <span
      className={cn(
        "inline-flex items-center gap-1 text-label-xs font-medium px-1.5 py-0.5 rounded shrink-0",
        config.className
      )}
    >
  ...
  return (
    ...
    >
      <div className="flex items-center gap-2">
        ...
        <span className="text-label text-foreground/80 truncate flex-1">{asset.filename}</span>
      ...
      {hasRegistry ? (
        ...
        <div className="flex items-center gap-1.5">
          <input
            ...
            className="flex-1 h-7 px-2 rounded bg-white/[0.03] border border-border/50 text-label text-foreground placeholder:text-muted-foreground/75 focus:outline-none focus:border-primary/40"
          />
          <button
    ...
    <div className="space-y-2">
      <div className="flex items-start gap-3 p-3 rounded-lg border border-border/40 bg-white/[0.02]">
        ...
        <div className="min-w-0 flex-1">
          ...
          <p className="text-label text-muted-foreground/85 leading-relaxed mt-0.5">
      ...
      >
        <Upload className="w-4 h-4 text-muted-foreground/85" />
        {!isUploading && (
          <span className="text-label text-muted-foreground/80">Choose one or more creative files (max 8 MB each)</span>
        )}
      </button>
      ...
      {errors.length > 0 && (
        <div className="flex items-start gap-2 p-2.5 rounded-lg border border-red-400/25 bg-red-400/[0.06]">
          <AlertTriangle className="w-3.5 h-3.5 text-red-400 shrink-0 mt-0.5" />
          <div className="text-label text-red-300 leading-relaxed space-y-0.5">
            {errors.map((msg, i) => (
              <p key={i}>{msg}</p>
            ))}
          </div>
      ...
      {linkNotices.length > 0 && (
        <div className="flex items-start gap-2 p-2.5 rounded-lg border border-amber-400/25 bg-amber-400/[0.06]">
          ...
          <div className="text-label text-amber-300 leading-relaxed space-y-0.5">
    ...
    return (
      <div className="space-y-3">
        <div className="rounded-lg border border-border/40 bg-white/[0.02] p-3 space-y-2">
          <div className="text-body font-semibold text-foreground">Review before finishing</div>
          <div className="space-y-1.5">
            ...
            {creativeAssets.length > 0 ? (
              creativeAssets.map((a) => (
                <div key={a.id} className="flex items-center gap-2 text-label">
              ...
              <div className="text-label text-muted-foreground/80">No creative files staged.</div>
            )}
          </div>
          <p className="text-label text-muted-foreground/75 leading-relaxed pt-1 border-t border-border/30">
