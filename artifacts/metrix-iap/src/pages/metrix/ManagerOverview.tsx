      ...
      <div className="px-6 py-5 space-y-6 max-w-6xl">
        ...
        <div>
          <div className="flex items-center justify-between mb-3">
            <h2 className="text-label font-mono uppercase tracking-widest text-muted-foreground/60">Bottom-line totals</h2>
            ...
            {events.map(([key, e]) => (
              <div key={key} className="rounded-lg border border-border/40 bg-white/[0.02] p-3.5">
                <div className="flex items-center gap-1.5 mb-2">
                  ...
                  <span className="text-label font-medium text-foreground leading-tight">{eventLabel(key)}</span>
                </div>
        ...
        <SectionCard title="Ad accounts" desc="Select an account to open its scoped intelligence platform.">
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
              ...
              return (
                ...
                >
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="text-body font-medium text-foreground leading-tight">{a.name}</div>
            ...
            >
              ...
              <div className="flex-1 min-w-0">
                <div className="text-body font-medium text-foreground/80 leading-tight">Add or connect an ad account</div>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-3">
              {data.recommendation_cards.map((c) => (
                <div key={c.id} className="rounded-xl border border-border/40 bg-white/[0.02] p-4 flex flex-col">
                  ...
                  <p className="text-body font-semibold text-foreground leading-snug">{c.title}</p>
                  <p className="text-body text-muted-foreground/70 mt-1 leading-relaxed">{c.rationale}</p>
                  <div className="flex items-start gap-1.5 mt-3 pt-3 border-t border-border/20">
                    ...
                    <p className="text-label text-foreground/75 leading-relaxed">{c.recommended_action}</p>
                  </div>
                  <button
                    onClick={() => selectAdAccount(c.account_id)}
                    className="mt-3 self-start inline-flex items-center gap-1 text-label font-medium text-primary/80 hover:text-primary transition-colors"
                  >
                    Open {accountName(c.account_id)} <ArrowRight className="w-3 h-3" />
                  </button>
                  {c.source_path && (
                    <p className="text-label-xs font-mono text-muted-foreground/60 mt-2">source · {c.source_path}</p>
                  )}
                </div>
              ))}
            </div>
