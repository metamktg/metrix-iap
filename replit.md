...
- Text-density standard: typography roles live in `artifacts/metrix-iap/src/pages/metrix/typography.ts` (`TYPE.label` 10px eyebrow / `TYPE.title` 13px / `TYPE.body` 12px / `TYPE.caption` 11px) and the `DenseText` clamp primitive in `shared.tsx` (CSS line-clamp + More/Less; `ExpandableText`/`ClampedProse` are aliases). Card prose composes from these roles — no half-pixel font sizes (10.5/11.5px etc.); the rulebook comment atop `shared.tsx` is authoritative.
