...
- **Do not enable Supabase's GitHub integration ("Deploy to production" on merge to `main`).** It tracks applied migrations in its own history table, independent of `_metrix_migrations` (the manual script's own checksum ledger) — running both against `supabase/migrations/` means two systems disagreeing about what's already applied, against a database holding real client data. `migrate:metrix-official` is the only supported path to production schema changes. If this ever needs to change, do it deliberately: enable Supabase branching (preview DB per PR) and a required-review policy first, dry-run a migration through a preview branch, and retire the manual script's role explicitly — don't run both at once.
