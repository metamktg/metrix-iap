...
- **The IAP Loop is execute-on-command, never automatic.** Every loop stage — analysis, strategy, briefs, and any future report-builder / creative-scan / optimization-loop stage — runs only from an explicit human action. No stage runs on a schedule, chains to auto-fire the next stage, or executes as a side effect of data arriving. Live Meta API pulls (`report_pulls`) and manual CSV/creative uploads ingest and refresh data continuously/on-demand, but **ingestion must never trigger a loop run** — data ingestion and loop execution are strictly separate. When a stage completes it may mark the next stage *runnable* and surface it; new or refreshed live/manual data may flag already-run downstream stages as *stale* — but the interface only ever recommends the next run and a human executes it. This is a hard product constraint: the interface must express the loop as explicit, per-stage execute-on-command steps (runnable / locked / running / complete / stale), and must visibly decouple "live data refreshed" from "loop executed" so a data pull never reads as having run the loop.
...
- IAP Loop execution is strictly execute-on-command. Pulling live account data from Meta (or uploading CSVs/creatives) must never start an analysis/strategy/brief/etc. run — ingestion and loop execution are separate concerns. The interface surfaces the next runnable stage and lets the user execute it deliberately, but never auto-runs anything and never runs in perpetuity. (Stated by the agency owner, 2026-07-17.)
