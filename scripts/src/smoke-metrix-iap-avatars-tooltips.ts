// Smoke check: Avatars page tooltip e2e — segment signal badge, placements
// accordion, and drawer cell-code chips.
//
// Boots the metrix-iap Vite dev server on an isolated port, waits for it to
// be ready, runs the Playwright spec at
// tests/e2e/metrix-iap-avatars-tooltips.spec.ts, then tears the server down.
...
async function runTests(): Promise<void> {
  return new Promise((resolve, reject) => {
    const specPath = path.join(
      repoRoot,
      "tests/e2e/metrix-iap-avatars-tooltips.spec.ts",
    );
    ...
    child.on("exit", (code) => {
      if (code === 0) {
        ...
        reject(
          new Error(
            `Avatars tooltips e2e tests failed (exit ${code})\n--- output ---\n${output || "(no output)"}`,
          ),
        );
      }
    });
...
async function main() {
  console.log(
    "Starting @workspace/metrix-iap dev server for Avatars tooltip e2e tests...",
  );
  ...
  try {
    ...
    console.log("\nRunning Avatars tooltip e2e tests...\n");
