// Smoke check: Metrix IAP first-run journey end-to-end (Playwright).
//
// Boots the metrix-iap Vite dev server on an isolated port, waits for it to
// be ready, runs the Playwright spec at
// tests/e2e/metrix-iap-first-run.spec.ts, then tears the server down.
...
async function runTests(): Promise<void> {
  return new Promise((resolve, reject) => {
    const specPath = path.join(
      repoRoot,
      "tests/e2e/metrix-iap-first-run.spec.ts",
    );
    ...
    child.on("exit", (code) => {
      if (code === 0) {
        ...
        reject(
          new Error(
            `first-run journey e2e tests failed (exit ${code})\n--- output ---\n${output || "(no output)"}`,
          ),
        );
      }
    });
...
async function main() {
  console.log(
    "Starting @workspace/metrix-iap dev server for the first-run journey e2e tests...",
  );
  ...
  try {
    ...
    console.log("\nRunning first-run journey e2e tests...\n");
    await runTests().catch((err) => {
      fail("first-run journey e2e tests failed", String(err?.message ?? err));
    });
  } finally {
    killGroup(server);
  }

  console.log("\nPASS  first-run journey e2e tests passed.");
  process.exit(0);
}
