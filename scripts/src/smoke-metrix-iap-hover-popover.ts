// Smoke check: MetricHoverPopover end-to-end (Playwright).
//
// Boots the metrix-iap Vite dev server on a private port, waits for it
// to be ready, runs the Playwright spec at
// tests/e2e/metrix-iap-hover-popover.spec.ts, then tears the server down.
...
const DEV_PORT = "15175"; // private port; won't collide with any other workflow
...
async function runTests(): Promise<void> {
  return new Promise((resolve, reject) => {
    const specPath = path.join(
      repoRoot,
      "tests/e2e/metrix-iap-hover-popover.spec.ts",
    );
    ...
    child.on("exit", (code) => {
      if (code === 0) {
        ...
        reject(
          new Error(
            `Hover-popover e2e tests failed (exit ${code})\n--- output ---\n${output || "(no output)"}`,
          ),
        );
      }
    });
...
async function main() {
  console.log(
    "Starting @workspace/metrix-iap dev server for hover-popover e2e tests...",
  );
  ...
  try {
    ...
    console.log("\nRunning MetricHoverPopover e2e tests...\n");
