  ...
  return new Promise((resolve, reject) => {
    ...
    child.on("exit", (code) => {
      if (code === 0) {
        ...
        reject(
          new Error(
            `SectionInfoIcon tooltip e2e tests failed (exit ${code})\n--- output ---\n${output || "(no output)"}`,
          ),
        );
      }
    });
...
async function main() {
  console.log(
    "Starting @workspace/metrix-iap dev server for SectionInfoIcon tooltip e2e tests...",
  );
  ...
  try {
    ...
    await runTests().catch((err) => {
      fail("SectionInfoIcon tooltip e2e tests failed", String(err?.message ?? err));
    });
  } finally {
    server.kill();
  }

  console.log("\nPASS  SectionInfoIcon tooltip e2e tests passed.");
  process.exit(0);
}
